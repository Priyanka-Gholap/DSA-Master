...
class GamificationService {
     ...
     */
    static async awardXP(userId, xpAmount, activityType, description) {
        try {
            // 1. Log Activity
     ...
     */
    static async updateGoalsProgress(userId, activityType) {
        ...
        for (const goal of goals) {
            ...
            if (increment) {
                ...
                if (isCompleted) {
                    ...
                    await this.awardXP(userId, 30, 'GOAL_COMPLETED', `Daily Goal Completed: "${goal.title}"`);
                }
            }
        }
    }
        ...
        for (const ach of allAchievements) {
            ...
            switch (ach.title) {
                ...
                case 'First Lesson Completed':
                    if (lessonsCompletedCount >= 1)
                        shouldUnlock = true;
                    break;
                case '10 Problems Solved':
                    if (problemsSolvedCount >= 10)
                        shouldUnlock = true;
                    break;
                case '50 Problems Solved':
                    if (problemsSolvedCount >= 50)
                        shouldUnlock = true;
                    break;
                case '100 Problems Solved':
                    if (problemsSolvedCount >= 100)
                        shouldUnlock = true;
                    break;
                case '7 Day Streak':
                    if (streakDays >= 7)
                ...
                case 'Graph Master':
                    ...
                    if (totalGraph > 0 && solvedGraph >= totalGraph)
                        shouldUnlock = true;
                    break;
                case 'DP Expert':
                    ...
                    if (solvedDP >= 1)
                        shouldUnlock = true; // Unlocks upon first DP problem solved
                    break;
            }
            if (shouldUnlock) {
                ...
                await this.awardXP(userId, ach.xpReward, 'SYSTEM', `Achievement Unlocked: "${ach.title}"`);
            }
        }
    }
}
